...
dist/libflac.wasm.js: $(FLAC_LIB) $(PREFILE) $(POSTFILE)
	$(EMCC) $(EMCC_DEF_OPT_LEVEL) $(EMCC_OPTS_WASM_DEFAULT) -sMINIMAL_RUNTIME_STREAMING_WASM_INSTANTIATION=1 -sSTRICT_JS=1 -sPRINTF_LONG_DOUBLE=1 -sWASM_BIGINT=1 -ffast-math -flto -sAGGRESSIVE_VARIABLE_ELIMINATION=1 -sELIMINATE_DUPLICATE_FUNCTIONS=1  -fno-rtti -fno-exceptions  -s MALLOC="emmalloc" -sSUPPORT_LONGJMP=0 -DNDEBUG -sASSERTIONS=0 --pre-js $(PREFILE) --post-js $(POSTFILE) $(FLAC_LIB) -o $@
